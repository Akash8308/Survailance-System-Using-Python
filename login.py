...
class login_page:
    ...
    def forgot_func(self):
        if self.email_entry.get()=="":
            ...
            try:
                connection = pymysql.connect(host="localhost", user="root", password="", database="employee")
                cur = connection.cursor()
    ...
    def change_pass(self):
        if self.email_entry.get() == "" or self.sec_ques.get() == "Select" or self.new_pass.get() == "":
            ...
            try:
                connection = pymysql.connect(host=cr.host, user=cr.user, password=cr.password, database=cr.database)
                cur = connection.cursor()
                cur.execute("select * from employee where email=%s and question=%s and answer=%s", (self.email_entry.get(),self.sec_ques.get(),self.ans.get()))
                row=cur.fetchone()
                ...
                if row == None:
                    ...
                    try:
                        cur.execute("update employee set password=%s where email=%s", (self.new_pass.get(),self.email_entry.get()))
                        connection.commit()
